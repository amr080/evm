...
describe("XFT Module System", function () {
  ...
  describe("TransferAgentModule", function () {
    it("Should settle purchase transaction", async function () {
      ...
      // Use blockchain time + future to ensure transaction date <= settlement date
      const block = await ethers.provider.getBlock("latest");
      const date = (block?.timestamp || 0) + 3600; // 1 hour in future
    ...
    it("Should settle liquidation transaction", async function () {
      ...
      let date = (block?.timestamp || 0) + 3600; // Future date
      ...
      date = (block?.timestamp || 0) + 7200; // Further future date
    ...
    it("Should distribute dividends", async function () {
      ...
      const price = ethers.parseUnits("1.0", 18);
      const block = await ethers.provider.getBlock("latest");
      const date = (block?.timestamp || 0) + 3600; // Future date
      await ta.connect(admin).settleTransactions([user1.address], [...txs], date, price);

      const balanceBefore = await mmf.balanceOf(user1.address);

      // Distribute dividends
      // Rate is int256: positive = dividend, negative = negative yield
      // Calculation: dividendAmount = balance * abs(rate), so rate should be a fraction
      // For 5%: rate = 0.05 * 1e18 = 5e16 (5% of balance)
