...
/**
 * @title XFT Tokenized Convertible Securities Protocol
    ...
    EIP712Upgradeable
{
    ...
    // Total token shares
    uint256 private _totalConverts;
    ...
    // Convert Errors
    error ConvertInvalidMintReceiver(address receiver);
    error ConvertInvalidBurnSender(address sender);
    error ConvertInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error ConvertInvalidRewardMultiplier(uint256 rewardMultiplier);
    error ConvertBlockedSender(address sender);
    error ConvertInvalidBlockedAccount(address account);
    error ConvertPausedTransfers();
     ...
     */
    function convertToConverts(uint256 amount) public view returns (uint256) {
     ...
     */
    function totalConverts() external view returns (uint256) {
        return _totalConverts;
    }
    ...
    function totalSupply() external view returns (uint256) {
        return convertToTokens(_totalConverts);
    }
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert ConvertInvalidMintReceiver(to);
        }

        _beforeTokenTransfer(address(0), to, amount);

        uint256 shares = convertToConverts(amount);
        _totalConverts += shares;

        unchecked {
            // Overflow not possible: shares + shares amount is at most totalConverts + shares amount
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert ConvertInvalidBurnSender(account);
        }

        _beforeTokenTransfer(account, address(0), amount);

        uint256 shares = convertToConverts(amount);
        uint256 accountConverts = sharesOf(account);

        if (accountConverts < shares) {
            revert ConvertInsufficientBurnBalance(account, accountConverts, shares);
        }

        unchecked {
            _shares[account] = accountConverts - shares;
            // Overflow not possible: amount <= accountConverts <= totalConverts.
            _totalConverts -= shares;
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        ...
        if (isBlocked(from)) {
            revert ConvertBlockedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert ConvertPausedTransfers();
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 shares = convertToConverts(amount);
        uint256 fromConverts = _shares[from];

        if (fromConverts < shares) {
            revert ERC20InsufficientBalance(from, fromConverts, shares);
        }

        unchecked {
            _shares[from] = fromConverts - shares;
            // Overflow not possible: the sum of all shares is capped by totalConverts, and the sum is preserved by
    ...
    function _blockAccount(address account) private {
        if (isBlocked(account)) {
            revert ConvertInvalidBlockedAccount(account);
        }
    ...
    function _setRewardMultiplier(uint256 _rewardMultiplier) private {
        if (_rewardMultiplier < _BASE) {
            revert ConvertInvalidRewardMultiplier(_rewardMultiplier);
        }
    ...
    function addRewardMultiplier(uint256 _rewardMultiplierIncrement) external onlyRole(ORACLE_ROLE) {
        if (_rewardMultiplierIncrement == 0) {
            revert ConvertInvalidRewardMultiplier(_rewardMultiplierIncrement);
        }
