...
/**
 * @title XFT Digital Loan Protocol
    ...
    EIP712Upgradeable
{
    ...
    // Loan Errors
    error LoanInvalidMintReceiver(address receiver);
    error LoanInvalidBurnSender(address sender);
    error LoanInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error LoanInvalidRewardMultiplier(uint256 rewardMultiplier);
    error LoanBlockedSender(address sender);
    error LoanInvalidBlockedAccount(address account);
    error LoanPausedTransfers();
     ...
     */
    function convertToLoans(uint256 amount) public view returns (uint256) {
    ...
    function totalSupply() external view returns (uint256) {
        return convertToTokens(_totalLoans);
    }
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert LoanInvalidMintReceiver(to);
        }

        _beforeTokenTransfer(address(0), to, amount);

        uint256 shares = convertToLoans(amount);
        _totalLoans += shares;

        unchecked {
            // Overflow not possible: shares + shares amount is at most totalLoans + shares amount
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert LoanInvalidBurnSender(account);
        }

        _beforeTokenTransfer(account, address(0), amount);

        uint256 shares = convertToLoans(amount);
        uint256 accountLoans = sharesOf(account);

        if (accountLoans < shares) {
            revert LoanInsufficientBurnBalance(account, accountLoans, shares);
        }

        unchecked {
            _shares[account] = accountLoans - shares;
            // Overflow not possible: amount <= accountLoans <= totalLoans.
            _totalLoans -= shares;
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        ...
        if (isBlocked(from)) {
            revert LoanBlockedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert LoanPausedTransfers();
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 shares = convertToLoans(amount);
        uint256 fromLoans = _shares[from];

        if (fromLoans < shares) {
            revert ERC20InsufficientBalance(from, fromLoans, shares);
        }

        unchecked {
            _shares[from] = fromLoans - shares;
            // Overflow not possible: the sum of all shares is capped by totalLoans, and the sum is preserved by
    ...
    function _blockAccount(address account) private {
        if (isBlocked(account)) {
            revert LoanInvalidBlockedAccount(account);
        }
    ...
    function _setRewardMultiplier(uint256 _rewardMultiplier) private {
        if (_rewardMultiplier < _BASE) {
            revert LoanInvalidRewardMultiplier(_rewardMultiplier);
        }
    ...
    function addRewardMultiplier(uint256 _rewardMultiplierIncrement) external onlyRole(ORACLE_ROLE) {
        if (_rewardMultiplierIncrement == 0) {
            revert LoanInvalidRewardMultiplier(_rewardMultiplierIncrement);
        }
