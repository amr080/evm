...
/**
 * @title Tokenized Equities Protocol
    ...
    EIP712Upgradeable
{
    ...
    // Share Errors
    error ShareInvalidMintReceiver(address receiver);
    error ShareInvalidBurnSender(address sender);
    error ShareInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error ShareInvalidRewardMultiplier(uint256 rewardMultiplier);
    error ShareBlockedSender(address sender);
    error ShareInvalidBlockedAccount(address account);
    error SharePausedTransfers();
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert ShareInvalidMintReceiver(to);
        }
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert ShareInvalidBurnSender(account);
        }
        ...
        if (accountShares < shares) {
            revert ShareInsufficientBurnBalance(account, accountShares, shares);
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        ...
        if (isBlocked(from)) {
            revert ShareBlockedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert SharePausedTransfers();
        }
    }
    ...
    function _blockAccount(address account) private {
        if (isBlocked(account)) {
            revert ShareInvalidBlockedAccount(account);
        }
    ...
    function _setRewardMultiplier(uint256 _rewardMultiplier) private {
        if (_rewardMultiplier < _BASE) {
            revert ShareInvalidRewardMultiplier(_rewardMultiplier);
        }
    ...
    function addRewardMultiplier(uint256 _rewardMultiplierIncrement) external onlyRole(ORACLE_ROLE) {
        if (_rewardMultiplierIncrement == 0) {
            revert ShareInvalidRewardMultiplier(_rewardMultiplierIncrement);
        }
