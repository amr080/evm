    ...
    IAdminTransfer
{
    ...
    uint256 public couponRate;           // Basis points (500 = 5%)
    bool public isActive;                // Prevents minting after maturity
    mapping(uint256 => bool) public couponPaid; // paymentDate => paid
    ModuleRegistry public moduleRegistry;
    bytes32 constant AUTHORIZATION_MODULE = keccak256("MODULE_AUTHORIZATION");
    EnumerableSet.AddressSet private accountsWithHoldings;
    ...
    event BondMatured(uint256 totalBurned);
    event CouponPaid(uint256 indexed paymentDate);
    event Mint(address indexed to, uint256 amount);
    event DistributeCoupon(
     ...
     * @param _couponRate Coupon rate in basis points (500 = 5%).
     * @param _moduleRegistry Address of the ModuleRegistry contract.
     */
    function initialize(
        string memory name_,
    ...
    /**
     * @notice Mints shares to the specified address. Used by TransferAgentModule.
     * @dev Calls _mint() and updates accountsWithHoldings set.
     * @param account The address to mint shares to.
     * @param shares The amount of shares to mint.
     */
    function mintBonds(address account, uint256 shares) external onlyRole(MINTER_ROLE) {
        require(isActive, "BOND_MATURED");
        uint256 amount = convertToTokens(shares);
    ...
    /**
     * @notice Burns shares from the specified address. Used by TransferAgentModule.
     * @dev Calls _burn() and updates accountsWithHoldings set.
     * @param account The address to burn shares from.
     * @param shares The amount of shares to burn.
     */
    function burnBonds(address account, uint256 shares) external onlyRole(BURNER_ROLE) {
        uint256 amount = convertToTokens(shares);
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount) private {
        ...
        // Update accountsWithHoldings set
        if (from != address(0) && sharesOf(from) == 0) {
            accountsWithHoldings.remove(from);
        }
        if (to != address(0) && sharesOf(to) > 0) {
            accountsWithHoldings.add(to);
        }
    }

    /**
     * @notice Transfers shares from one address to another. Used by TransferAgentModule.
     * @dev Checks WRITE_ACCESS_TOKEN role via ModuleRegistry and calls _transfer().
     * @param from The address to transfer shares from.
     * @param to The address to transfer shares to.
     * @param amount The amount of shares to transfer.
     */
    function transferShares(address from, address to, uint256 amount) external override {
        require(
            AccessControlUpgradeable(
        ...
        uint256 tokenAmount = convertToTokens(amount);
    ...
    /**
     * @notice Returns the share holdings for the specified account.
     * @dev Returns the balance of the account.
     * @param account The account to check.
     * @return The balance of the account.
     */
    function getShareHoldings(address account) external view override returns (uint256) {
        return convertToTokens(sharesOf(account));
    }

    /**
     * @notice Checks if the account has enough holdings.
     * @dev Returns true if balance >= amount.
     * @param account The account to check.
     * @param amount The amount to check against.
     * @return True if the account has enough holdings.
     */
    function hasEnoughHoldings(address account, uint256 amount) external view override returns (bool) {
        return convertToTokens(sharesOf(account)) >= amount;
    }

    /**
     * @notice Updates the holder in the accountsWithHoldings list.
     * @dev Adds account if balance > 0, removes if balance = 0.
    ...
    /**
     * @notice Removes empty holder from the accountsWithHoldings list.
     * @dev Removes account if balance = 0.
    ...
    /**
     * @notice Settles the bond at maturity by burning all tokens from all holders.
     * @dev Iterates through all holders and burns their balances, then sets isActive = false.
     * Can only be called by DEFAULT_ADMIN_ROLE after maturity date.
     */
    function settleMaturity() external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (block.timestamp < maturityDate) {
            revert BondNotMature();
        }
        if (!isActive) {
            revert BondAlreadySettled();
        }
        
        isActive = false;
        uint256 totalBurned = 0;
        
        // Burn from all holders - collect addresses first to avoid index issues
        uint256 holderCount = accountsWithHoldings.length();
        address[] memory holders = new address[](holderCount);
        for (uint256 i = 0; i < holderCount; ) {
            holders[i] = accountsWithHoldings.at(i);
            unchecked { ++i; }
        }
        
        // Burn from all collected holders and emit MaturityRedemption
        for (uint256 i = 0; i < holderCount; ) {
            address holder = holders[i];
            uint256 balance = convertToTokens(sharesOf(holder));
            if (balance > 0) {
                emit MaturityRedemption(holder, maturityDate, balance);
        ...
        // Clear all holders
        for (uint256 i = 0; i < holderCount; ) {
            accountsWithHoldings.remove(holders[i]);
            unchecked { ++i; }
        }
        
        emit BondMatured(totalBurned);
    }

    /**
     * @notice Marks a coupon payment date as paid and distributes coupon to all holders.
     * @dev Can only be called by DEFAULT_ADMIN_ROLE. Emits DistributeCoupon for each holder.
     * @param paymentDate The payment date as Unix timestamp.
     */
    function markCouponPaid(uint256 paymentDate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        couponPaid[paymentDate] = true;
        emit CouponPaid(paymentDate);
        
        // Emit DistributeCoupon for each holder
        uint256 holderCount = accountsWithHoldings.length();
        for (uint256 i = 0; i < holderCount; ) {
            address holder = accountsWithHoldings.at(i);
            uint256 principalBalance = convertToTokens(sharesOf(holder));
            if (principalBalance > 0) {
                // Calculate semi-annual coupon (couponRate is in basis points, divide by 2 for semi-annual)
                uint256 couponAmount = (principalBalance * couponRate) / 10000 / 2;
                emit DistributeCoupon(holder, paymentDate, couponRate, couponAmount, principalBalance);
            }
            unchecked { ++i; }
        }
    }
     ...
     * Original gap: 42. Added 6 new state variables, so gap is now 36.
     */
