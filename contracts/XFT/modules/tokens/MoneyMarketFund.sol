    ...
    IAdminTransfer
{
    ...
    function transferShares(
        ...
        // Transfer using _transfer - policy check will allow it because
        // we need to update _checkTransferPolicy to allow admin-initiated transfers
    ...
    function _checkTransferPolicy(
        ...
        // Allow transfers initiated by modules with WRITE_ACCESS_TOKEN (admin-initiated)
        if (
            AccessControlUpgradeable(
                moduleRegistry.getModuleAddress(AUTHORIZATION_MODULE)
            ).hasRole(keccak256("WRITE_ACCESS_TOKEN"), _msgSender())
        ) {
            // Admin-initiated transfers: both from and to must be authorized (or from can be admin)
            if (from != address(0)) {
                require(
                    IAuthorization(
                        moduleRegistry.getModuleAddress(AUTHORIZATION_MODULE)
            ...
            return; // Allow admin-initiated transfer
        }
