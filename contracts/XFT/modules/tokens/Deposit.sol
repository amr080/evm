...
/**
 * @title XFT Digital Deposit Protocol
    ...
    EIP712Upgradeable
{
    ...
    // Deposit Errors
    error DepositInvalidMintReceiver(address receiver);
    error DepositInvalidBurnSender(address sender);
    error DepositInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error DepositInvalidRewardMultiplier(uint256 rewardMultiplier);
    error DepositBlockedSender(address sender);
    error DepositInvalidBlockedAccount(address account);
    error DepositPausedTransfers();
     ...
     */
    function convertToDeposits(uint256 amount) public view returns (uint256) {
    ...
    function totalSupply() external view returns (uint256) {
        return convertToTokens(_totalDeposits);
    }
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert DepositInvalidMintReceiver(to);
        }

        _beforeTokenTransfer(address(0), to, amount);

        uint256 shares = convertToDeposits(amount);
        _totalDeposits += shares;

        unchecked {
            // Overflow not possible: shares + shares amount is at most totalDeposits + shares amount
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert DepositInvalidBurnSender(account);
        }

        _beforeTokenTransfer(account, address(0), amount);

        uint256 shares = convertToDeposits(amount);
        uint256 accountDeposits = sharesOf(account);

        if (accountDeposits < shares) {
            revert DepositInsufficientBurnBalance(account, accountDeposits, shares);
        }

        unchecked {
            _shares[account] = accountDeposits - shares;
            // Overflow not possible: amount <= accountDeposits <= totalDeposits.
            _totalDeposits -= shares;
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        ...
        if (isBlocked(from)) {
            revert DepositBlockedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert DepositPausedTransfers();
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 shares = convertToDeposits(amount);
        uint256 fromDeposits = _shares[from];

        if (fromDeposits < shares) {
            revert ERC20InsufficientBalance(from, fromDeposits, shares);
        }

        unchecked {
            _shares[from] = fromDeposits - shares;
            // Overflow not possible: the sum of all shares is capped by totalDeposits, and the sum is preserved by
    ...
    function _blockAccount(address account) private {
        if (isBlocked(account)) {
            revert DepositInvalidBlockedAccount(account);
        }
    ...
    function _setRewardMultiplier(uint256 _rewardMultiplier) private {
        if (_rewardMultiplier < _BASE) {
            revert DepositInvalidRewardMultiplier(_rewardMultiplier);
        }
    ...
    function addRewardMultiplier(uint256 _rewardMultiplierIncrement) external onlyRole(ORACLE_ROLE) {
        if (_rewardMultiplierIncrement == 0) {
            revert DepositInvalidRewardMultiplier(_rewardMultiplierIncrement);
        }
